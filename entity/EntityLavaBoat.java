...
public class EntityLavaBoat extends Entity {
    ...
    public void onUpdate() {
        ...
        for (int i = 0; i < b0; ++i) {
            double d1 = this.boundingBox.minY + (this.boundingBox.maxY - this.boundingBox.minY) * (i + 0) / b0 - 1.1;
            ...
            if (this.worldObj.isAABBInMaterial(axisalignedbb, Material.lava)) {
                d0 += 1.0D / b0;
            } else if (mod_LavaBoat.canSwimInWater && this.worldObj.isAABBInMaterial(axisalignedbb, Material.water)) {
                d0 += 1.0D / b0;
            }
        }
